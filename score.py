...
from format import convert_input_type
...
def get_attributions(
    ...
    with torch.no_grad():
        if task =='rna_saluki':
            seqs_tran = transform_samples_saluki(seqs)
            # for i in range(len(seqs_tran)):
            #     X_ = seqs_tran[i : i + 1].to(device)  # 1, 4, L
            #
            #     if method == "deepshap":
            ...
            #     else:
            #         # B, _, L = X_.shape
            #         # # Create a zero tensor of shape (B, 2, L)
            #         # zeros = torch.zeros(B, 2, L, dtype=torch.float32).to(device)
            #         # # Concatenate the original tensor with the zero tensor along the second dimension (channels)
            #         # X_ = torch.cat((X_, zeros), dim=1)
            #         attr = attributer.attribute(X_)[:, :4, :50]
            #
            #     attributions.append(attr.cpu().numpy())
            return attributer.attribute(seqs_tran.to(device))[:, :4, :50].cpu().numpy()
        else:
            return attributer.attribute(seqs.to(device)).cpu().numpy()
...
def transform_samples_saluki(samples, num_classes=4, final_length=12288):
    ...
    one_hot_samples = samples.transpose(1, 2)
