...
def run(args, rank=None):
    ...
    if args.model == 'enformer':
        ...
        model = BaseModel(embedding=common_trunk, head=reg_head, cdq=args.cdq, batch_size=args.batch_size, val_batch_num=1)  #args.val_batch_num
    ...
    model.cuda()
    model.eval()
    predictions, targets, my_pearsonr_scores = model.evaluation(batch_num=32)
    print("my_pearsonr_scores", my_pearsonr_scores)

    # Concatenate all batches
    pred_all = torch.cat(predictions).view(-1).cpu()
    true_y_all = torch.cat(targets).view(-1).cpu()

    # Calculate Pearson correlation coefficient
    pearson_r, _ = pearsonr(pred_all.numpy(), true_y_all.numpy())

    # Create a scatter plot
    plt.figure(figsize=(10, 6))
    sns.scatterplot(x=pred_all.numpy(), y=true_y_all.numpy())
    plt.xlabel('Predicted Values')
    plt.ylabel('True Values')
    plt.title(f'Scatter Plot of Predictions vs True Values (PearsonR: {pearson_r:.2f})')
    plt.grid(True)

    # Save the plot
    plt.savefig("pred_vs_true_scatter.png")
    wandb.log({"pred_vs_true_scatter": wandb.Image("pred_vs_true_scatter.png")})
